setwd("E:/backup/SmallMolecules/healthy_data/diseases_analysis/CRC/ML/")
...
# load data
load("ML_2step_CRC_naturalCompounds.RData")
model <- ranger_mod
...
gathered_final_model_shap <-
  ...
  mutate(Group = raw_data$Group %>% factor(levels = c("control", "CRC"))) %>%
  ungroup() %>% 
...
final_model_shap_plot_left <-
  ggplot(data = gathered_final_model_shap_contribution %>% 
           mutate(Sign = ifelse(glm_slope > 0, "Control", "CRC"))%>% 
  ...
  scale_fill_manual(values = c("Control" = "#ffc685","CRC" = "#c9512c")) +
  ...
  lapply(top_features, function(unique_key){
    ...
    current_plot <- 
      ...
      theme(panel.grid = element_blank(),
            ...
            legend.title = element_blank(),
            aspect.ratio = 1) +
      scale_color_manual(values=c("control" = "#ffc685","CRC" = "#c9512c"))
    
    return(current_plot)
  })

ggpubr::ggarrange(plotlist = list_of_dependence_plot, ncol = 5, nrow = 6, common.legend = T, align = "hv")