...
gathered_final_model_shap <-
  ...
  mutate(Group = raw_data$Group %>% factor(levels = c("Control", "IBD"))) %>%
  ungroup() %>% 
...
final_model_shap_plot_left <-
  ggplot(data = gathered_final_model_shap_contribution %>% 
           mutate(Sign = ifelse(glm_slope > 0, "Control", "IBD")) %>% 
  ...
  scale_fill_manual(values = c("Control" = "#a9deff","IBD"="#558adf")) +
  ...
  lapply(top_features, function(unique_key){
    ...
    current_plot <- 
      ...
      theme(panel.grid = element_blank(),
            ...
            legend.position = "top",
            legend.title = element_blank()) +
      scale_color_manual(values=c("Control" = "#a9deff", "IBD" = "#558adf"))
    
    return(current_plot)
  })

ggpubr::ggarrange(plotlist = list_of_dependence_plot, ncol = 5, nrow = 4, common.legend = T, align = "hv")