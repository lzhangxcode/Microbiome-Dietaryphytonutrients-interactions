# the aim of this script is to prepare the correlation figure for the mice DNA data
# author: lu.zhang 
# date: 2024.08.04 
...
# calculate day6 wilcox effectsize 
...
hescore_metag_straw_clr_G2G3 <- read_tsv("./HE_correlation/Total_score_EC_HEscore_cor_tibble.tsv") %>% 
  filter(P < 0.05) %>%
  mutate(R_direction = case_when(R > 0 ~ "positive", R < 0 ~ "negative")) 

hescore_metag_straw_clr_G2G3_full <- read_tsv("./HE_correlation/Total_score_EC_HEscore_cor_tibble.tsv")

# abundance data 
metag_clr <- read_tsv("./metagenomics_join_enzyme_clr.tsv")
...
overlap_direciton_same_trend <- read_tsv("./strawberry_summary_sig.EC_day6andday18_metagenomics_overlap_direction_only_one_groups.tsv")
...
ec_compounds_metag_g2g3 <- read_tsv("./EC_strawberry_both.tsv") %>% 
  select(EC, Compounds) %>% 
  unique()
...
setwd("./summary_sigEC_G2G3/Figure")

pdf("metagenomics_DNAv2.pdf", height = 7*1.5, width = 7*1.5)
...
# correlation figure for the 4.1.1.11 

##############################################################################################

# I need the correlation figure for the 4.1.1.11 for the metagenomics with HE/DAI score 
...
p_cor <- ggscatter(combine_tibble_for_cor_plot, title = target_ec, x = "EC_Abundance", y = "DAI_score", size = 4,                                               
  ...
  xlab("Abundance") + ylab("Histological score")

ggsave("./HE_correlation/total_score_4.1.1.11_DNA.pdf", p_cor, width = 5.5)



rm(list = ls())

load("./G2G3_correlation/correlation_strawberry.RData")

library(ggpubr)
target_ec <- "4.1.1.11"
combine_tibble_for_cor_plot <- tibble(EC_Abundance = EC_mice_unstratified_d18_temp[target_ec,] %>% as.numeric() %>% as.vector(),                                                                                    DAI_score = DAI_score_d18_order$D18_DAIscore %>% as.numeric()) %>% 
  mutate(sample = colnames(EC_mice_unstratified_d18_temp)) %>% 
  left_join(., Group_d18)
p_value <- EC_daiscore_cor_tibble_q$P[EC_daiscore_cor_tibble_q$EC == target_ec] %>% round(., digits=5)
r_value <- EC_daiscore_cor_tibble_q$R[EC_daiscore_cor_tibble_q$EC == target_ec] %>% round(., digits=5)
combine_tibble_for_cor_plot$Group <- combine_tibble_for_cor_plot$group
combine_tibble_for_cor_plot$Group[combine_tibble_for_cor_plot$Group == "G2"] <- "DSS+Vehicle"
combine_tibble_for_cor_plot$Group[combine_tibble_for_cor_plot$Group == "G3"] <- "DSS+Strawberry"
...
p_cor <- ggscatter(combine_tibble_for_cor_plot, title = target_ec, x = "EC_Abundance", y = "DAI_score", size = 4,                                               
  ...
  ggtitle(target_ec) +
  scale_color_manual(labels = c("DSS+Strawberry", "DSS+Vehicle"), values=c("#1b5d9d", "#00695d")) + geom_point(size = 4, aes(color = Group)) + 
  xlab("Abundance") + ylab("DAI score")

ggsave("./G2G3_correlation/daiscore_4.1.1.11_DNA.pdf", p_cor, width = 5.5)
