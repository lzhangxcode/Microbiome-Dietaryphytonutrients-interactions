# summary of the sig.ECs that are sig.diff between strawberry and non-strawberry group at day6 and day18 in DNA level 
# author: Lu.Zhang
# date: 2024.July.29
...
# check the ones that are either sig.diff at day6 or sig.diff at day18 
...
metag_cpm <- read_tsv("./metagenomics_join_enzyme_cpm.tsv")
colnames(metag_cpm)[1] <- "EC"
colnames(metag_cpm) <- str_remove(colnames(metag_clr), ".clean_Abundance-CPM")
...
write_tsv(overlap_direciton_same_trend, "./strawberry_summary_sig.EC_day6andday18_metagenomics_overlap_direction_only_one_groups.tsv")
